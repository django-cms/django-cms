...
class Form(forms.BaseForm):
        ...
        if not domains and domain:
            domains = {
                1: {
                    "domain": domain,
                    "aliases": [
                        d.strip() for d in domain_aliases.split(",") if d.strip()
                    ],
                    "redirects": [
                        d.strip() for d in domain_redirects.split(",") if d.strip()
                    ],
                }
            }
    ...
    def i18n_settings(self, data, settings, env):
        ...
        settings["LANGUAGES"] = [
            (code, settings["ALL_LANGUAGES_DICT"][code]) for code in languages
        ]
    ...
    def migration_settings(self, settings, env):
        ...
        mcmds.append(
            "python manage.py migrate --list --noinput && python manage.py migrate --noinput && python manage.py migrate --list --noinput"
        )
