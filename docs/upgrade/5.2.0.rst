...
``site`` is a ``django.contrib.sites.models.Site`` instance -- the one the
permissions were computed for. Passing the wrong site is what the fix prevents,
so pass the site the request is being served for, not
``Site.objects.get_current()`` by reflex.

``clear_user_permission_cache(user)`` is unchanged, and now clears the user's
entries for every site.

This is an internal API, but a project or package that warms or inspects the
permission cache itself has to be updated: the old two-argument calls raise
``TypeError``.
