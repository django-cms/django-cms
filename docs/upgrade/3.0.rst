...
``PlaceholderAdmin`` now is mixin class and not derived from ``admin.ModelAdmin`` anymore.
Therefore when defining an admin class using ``PlaceholderAdmin``, additionally derive that
class from ``admin.ModelAdmin``.
