...
The largest major change is we've removed revision support from the core.
This means users can no longer undo/redo/recover pages.
...
* Added the possibility to copy pages in the Page Tree by drag'n'drop.
...
The permissions system was heavily refactored.
As a result, several internal functions and methods were removed or changed.
...
.. warning:: Manual plugin rendering is not a public API, as such it's subject to change
             without notice.
...
Like a plugin, to render a plugin programmatically, you will need a context and request object.


.. warning:: Manual placeholder rendering is not a public API, as such it's subject to change
             without notice.
