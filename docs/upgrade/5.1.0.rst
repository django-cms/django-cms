...
* Changed the default primary key field from ``AutoField`` to ``BigAutoField``.
* Apphooks now by default allow allowing access to the page content object's placeholders on
