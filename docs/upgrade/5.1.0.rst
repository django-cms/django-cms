...
* ``Page`` method ``page.remove_language()`` and ``page.update_languages()`` are deprecated and will be removed in django CMS 6.0.
...
* ``BaseRenderer`` method ``renderer.get_placeholder_toolbar_js()`` has a keyword argument ``page`` which will be
  removed in django CMS 6.0. It can already be savely removed from all calls.
