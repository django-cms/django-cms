...
We have tested the CMS interface extensively, but will be very keen to have feedback from other users
...
* We have moved our ``widgets.py`` JavaScript to ``static/cms/js/widgets``.


Code formatting
---------------

We have switched from tabs (in some places) to four spaces *everywhere*. See :ref:`contributing-code`
...
or developers). We have also adopted `CSSComb <http://csscomb.com>`_.


``.editorconfig`` file
----------------------

We have added a ``.editorconfig`` (at the root of the project) to provide cues to text editors.
