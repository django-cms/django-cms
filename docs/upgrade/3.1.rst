...
non-superuser cannot enter structure mode anymore: this allow more strict workflows which only
enable content update for certain users.
This will require that **any** non-superuser which is supposed to be allowed to use
structure mode, must be given the new permission.
