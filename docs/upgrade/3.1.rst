...
* Old-style plugin table names are no longer supported. For any custom plugin which has not
  been ported to new-style plugin table names, any plugin table name must be migrated to
  the standard table names layout;
...
* The migration for mptt to treebeard is handled by the django CMS migrations, thus apply
