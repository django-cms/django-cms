...
class PagesTestCase(TestCase):
    ...
    def test_09_copy_page(self):
        ...
        response = self.client.post('/admin/cms/page/add/', page_data)
        page_data = self.get_new_page_data()
        response = self.client.post('/admin/cms/page/add/?target=4&position=first-child&site=1', page_data)
        page_data = self.get_new_page_data()
        response = self.client.post('/admin/cms/page/1/copy-page/', {'target':5, 'position':'first-child'})
        self.assertEqual(response.status_code, 200)
        print dir(response)
