...
def handle_pagecontent_post_save(sender, instance, created, **kwargs):
    ...
    with an apphook causes NoReverseMatch exceptions until the server is restarted.
    """
    if created and instance.page.application_urls:
        # A new language variant was created for a page with an apphook
        # Trigger URL reloading to make apphook URLs available for this language
        logger.debug(
            f"New language variant '{instance.language}' created for page "
            f"'{instance.page}' with apphook '{instance.page.application_urls}'. "
            f"Triggering URL reload."
        )
        set_restart_trigger()