...
var Plugin = new Class({
    ...
    _setupUI: function setupUI(container) {
        ...
        if (wrapper.length > 1 && container.match(/cms-plugin/)) {
            ...
            var contentWrappers = wrapper.toArray().reduce(function (wrappers, elem, index) { // elsint-disable-line
                if (index === 0) {
            ...
            contents = contentWrappers.map(function (items) {
            ...
            if (contents.length) {
                ...
                contents = contents.reduce(function (collection, items) {
                    var update = collection.add(items);
