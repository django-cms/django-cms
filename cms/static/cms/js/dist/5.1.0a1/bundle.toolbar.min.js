...
class Clipboard {
  ...
  _handleExternalUpdate(value) {
    ...
    _cms_plugins__WEBPACK_IMPORTED_MODULE_3__["default"]._updateClipboard();
    CMS._instances.push(new _cms_plugins__WEBPACK_IMPORTED_MODULE_3__["default"](`cms-plugin-${clipboardData.data.plugin_id}`, clipboardData.data));
...
class Plugin {
   ...
   */
  _setupUI(container) {
    const wrapper = jquery__WEBPACK_IMPORTED_MODULE_2___default()(`.${container}`);
    let contents;
  ...
  _setPluginStructureEvents() {
    ...
    this.ui.draggable.off('cms-paste-plugin-update').on('cms-paste-plugin-update', function (e, eventData) {
      ...
      var dragitem = jquery__WEBPACK_IMPORTED_MODULE_2___default()(`.cms-draggable-${eventData.id}:last`);
  ...
  _getPluginAddPosition() {
    if (this.options.type === 'placeholder') {
      return jquery__WEBPACK_IMPORTED_MODULE_2___default()(`.cms-dragarea-${this.options.placeholder_id} .cms-draggable`).length + 1;
    }
  ...
  movePlugin(opts) {
    ...
    const dragitem = jquery__WEBPACK_IMPORTED_MODULE_2___default()(`.cms-draggable-${options.plugin_id}:last`);
  ...
  destroy() {let options = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
    ...
    _cms_base__WEBPACK_IMPORTED_MODULE_9__.$document.off(`.${this.uid}`);
    _cms_base__WEBPACK_IMPORTED_MODULE_9__.$window.off(`.${this.uid}`);
  }
  ...
  _updatePlaceholderCollapseState() {
    ...
    const el = jquery__WEBPACK_IMPORTED_MODULE_2___default()(`.cms-dragarea-${this.options.placeholder_id} .cms-dragbar-title`);
    var settings = CMS.settings;
...
Plugin._initializeGlobalHandlers = function _initializeGlobalHandlers() {
  ...
  setTimeout(function () {
    ...
    if (clipboardDraggable.length) {
      pluginData = CMS._plugins.find(
        (_ref12) => {let [desc] = _ref12;return desc === `cms-plugin-${CMS.API.StructureBoard.getId(clipboardDraggable)}`;}
      )[1];
  ...
  _cms_base__WEBPACK_IMPORTED_MODULE_9__.$document.on(`${Plugin.pointerOverAndOut} ${Plugin.touchStart}`, '.cms-plugin', function (e) {
    ...
    var placeholderId = CMS.API.StructureBoard.getId(jquery__WEBPACK_IMPORTED_MODULE_2___default()(`.cms-draggable-${id}`).closest('.cms-dragarea'));
...
{let { successTimeout = 200, seeThrough = false, delay = 1500, prominent = false } = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
  ...
  if (successTimeout) {
    setTimeout(() => {
      jquery__WEBPACK_IMPORTED_MODULE_2___default()(`.cms-plugin-overlay-${pluginId}`).fadeOut(successTimeout, function () {
        jquery__WEBPACK_IMPORTED_MODULE_2___default()(this).remove();
      });
    }, delay);
  }
...
Plugin._removeHighlightPluginContent = function (pluginId) {
  jquery__WEBPACK_IMPORTED_MODULE_2___default()(`.cms-plugin-overlay-${pluginId}[data-success-timeout=0]`).remove();
};
...
Plugin._updatePluginPositions = function (placeholder_id) {
  ...
  const plugins = jquery__WEBPACK_IMPORTED_MODULE_2___default()(`.cms-dragarea-${placeholder_id} .cms-draggable`).toArray();
...
class StructureBoard {
  ...
  _requestMode(mode) {
    ...
    if (!this[`_request${mode}`]) {
      this[`_request${mode}`] = jquery__WEBPACK_IMPORTED_MODULE_0___default().ajax({
        url: url.toString(),
        method: 'GET'
      }).then((markup) => {
  ...
  _drag() {let elem = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : this.ui.sortables;
    ...
    nestedSortable({
      ...
      update: function (event, ui) {
        ...
        const plugin = jquery__WEBPACK_IMPORTED_MODULE_0___default()(`.cms-draggable-${id}`);
        const eventData = {
          id: id
        };
  ...
  _updateSingleContent(content) {
    ...
    let nextEl = jquery__WEBPACK_IMPORTED_MODULE_0___default()(`:not(template).cms-plugin.cms-plugin-${content.pluginIds[0]}.cms-plugin-start`);
    ...
    content.pluginIds.forEach((id) => {
      jquery__WEBPACK_IMPORTED_MODULE_0___default()(`:not(template).cms-plugin.cms-plugin-${id}`).remove();
      jquery__WEBPACK_IMPORTED_MODULE_0___default()(`script[data-cms-plugin]#cms-plugin-${id}`).remove();
    });
  ...
  _findNextElement(position, placeholder_id, excludedPlugins) {
    let nextEl = jquery__WEBPACK_IMPORTED_MODULE_0___default()(`div.cms-placeholder.cms-placeholder-${placeholder_id}`);
    ...
    if (nextPlugins.length > 0) {
      ...
      nextEl = jquery__WEBPACK_IMPORTED_MODULE_0___default()(`.cms-plugin.cms-plugin-${nextPluginId}.cms-plugin-start`);
    }
    return nextEl;
  }
  ...
  _loadToolbar() {
    const placeholderIds = (0,_cms_toolbar__WEBPACK_IMPORTED_MODULE_3__.getPlaceholderIds)(CMS._plugins).map((id) => `placeholders[]=${id}`).join('&');
  ...
  handleMovePlugin(data) {
    if (data.plugin_parent) {
      if (data.plugin_id) {
        const draggable = jquery__WEBPACK_IMPORTED_MODULE_0___default()(`.cms-draggable-${data.plugin_id}:last`);
      ...
      jquery__WEBPACK_IMPORTED_MODULE_0___default()(`.cms-draggable-${data.plugin_parent}`).html('').replaceWith(data.html);
    } else {
      // the one in the clipboard is first, so we need to take the second one,
      // that is already visually moved into correct place
      let draggable = jquery__WEBPACK_IMPORTED_MODULE_0___default()(`.cms-draggable-${data.plugin_id}:last`);
      ...
      if (!draggable.closest('.cms-draggables').parent().is(`.cms-dragarea-${data.placeholder_id}`)) {
        ...
        const placeholderDraggables = jquery__WEBPACK_IMPORTED_MODULE_0___default()(`.cms-dragarea-${data.placeholder_id} > .cms-draggables`);
      ...
      if (draggable.length) {
        ...
        // copy from language
        jquery__WEBPACK_IMPORTED_MODULE_0___default()(`.cms-dragarea-${data.target_placeholder_id} > .cms-draggables`).append(data.html);
      }
    }
  ...
  handleAddPlugin(data) {
    if (data.plugin_parent) {
      jquery__WEBPACK_IMPORTED_MODULE_0___default()(`.cms-draggable-${data.plugin_parent}`).replaceWith(data.structure.html);
    } else {
      // the one in the clipboard is first
      jquery__WEBPACK_IMPORTED_MODULE_0___default()(`.cms-dragarea-${data.placeholder_id} > .cms-draggables`).append(data.structure.html);
    }
  ...
  handleEditPlugin(data) {
    if (data.plugin_parent) {
      ...
      jquery__WEBPACK_IMPORTED_MODULE_0___default()(`.cms-draggable-${data.plugin_id}`).replaceWith(data.structure.html);
    }
  ...
  handleDeletePlugin(data) {
    ...
    deletedPluginIds.forEach(function (pluginId) {
      if (!contentData.content) {
        jquery__WEBPACK_IMPORTED_MODULE_0___default()(`.cms-plugin.cms-plugin-${pluginId}`).remove(); // Remove from content
      }
      jquery__WEBPACK_IMPORTED_MODULE_0___default()(`script[data-cms-plugin]#cms-plugin-${pluginId}`).remove(); // Remove script elements
      (0,lodash_es_remove_js__WEBPACK_IMPORTED_MODULE_7__["default"])(CMS._plugins, (settings) => settings[0] === `cms-plugin-${pluginId}`);
  ...
  handleClearPlaceholder(data) {
    ...
    deletedIds.forEach((id) => {
      (0,lodash_es_remove_js__WEBPACK_IMPORTED_MODULE_7__["default"])(CMS._plugins, (settings) => settings[0] === `cms-plugin-${id}`);
      (0,lodash_es_remove_js__WEBPACK_IMPORTED_MODULE_7__["default"])(
        CMS._instances,
        (instance) => instance.options.plugin_id && Number(instance.options.plugin_id) === Number(id)
      );

      jquery__WEBPACK_IMPORTED_MODULE_0___default()(`.cms-draggable-${id}`).remove();
    });
  ...
  static actualizePluginCollapseStatus(pluginId) {
    const el = jquery__WEBPACK_IMPORTED_MODULE_0___default()(`.cms-draggable-${pluginId}`);
...
const query = function (selector) {let root = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : document;return root?.querySelector(selector);};
