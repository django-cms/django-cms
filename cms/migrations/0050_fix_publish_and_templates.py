...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for page in orm['cms.Page'].objects.filter(publisher_public_id__gt=0, publisher_is_draft=True):
            public_page = page.publisher_public
            public_titles = public_page.title_set.all()
            for public_title in public_titles:
                public_title.publisher_is_draft = False
                title = orm['cms.Title'].objects.get(language=public_title.language, page=page)
                public_title.publisher_public = title
                public_title.template = page.template
                public_title.save()
                title.publisher_public = public_title
    ...
    def backwards(self, orm):
        for page in orm['cms.Page'].objects.filter(publisher_public_id__gt=0, publisher_is_draft=True):
            public_page = page.publisher_public
            draft_titles = page.title_set.all()
            for title in draft_titles:
                page.template = title.template
                page.publication_date = datetime.datetime.now()
                page.save()
                public_page.template = title.template
                public_page.publication_date = datetime.datetime.now()
                public_page.save()
