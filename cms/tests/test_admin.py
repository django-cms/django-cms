...
class AdminTestsBase(CMSTestCase):
    ...
    def _get_staff_user(self, use_global_permissions=True):
        ...
        if get_user_model().USERNAME_FIELD == "email":
            normal_guy = get_user_model().objects.create_user(
                USERNAME, "test@test.com", "test@test.com"
            )
        else:
            normal_guy = get_user_model().objects.create_user(
...
class AdminTestCase(AdminTestsBase):
    ...
    def test_2apphooks_with_same_namespace(self):
        ...
        page = create_page(
            PAGE1, "nav_playground.html", "en", site=current_site, created_by=admin_user
        )
        page2 = create_page(
            PAGE2, "nav_playground.html", "en", site=current_site, created_by=admin_user
        )
        ...
        with self.login_user_context(admin_user):
            resp = self.client.post(
                base.URL_CMS_PAGE_ADVANCED_CHANGE % page.pk, page_data
            )
            self.assertEqual(resp.status_code, 302)
            self.assertEqual(
                Page.objects.filter(application_namespace="space1").count(), 1
            )
            resp = self.client.post(
                base.URL_CMS_PAGE_ADVANCED_CHANGE % page2.pk, page_data
            )
    ...
    def test_delete(self):
        admin_user = self.get_superuser()
        create_page(
            "home", "nav_playground.html", "en", created_by=admin_user, published=True
        )
        page = create_page(
            "delete-page",
            "nav_playground.html",
    ...
    def test_delete_translation(self):
        ...
        create_title(
            "de", "delete-page-translation-2", page, slug="delete-page-translation-2"
        )
        create_title(
            "es-mx",
            "delete-page-translation-es",
            page,
            slug="delete-page-translation-es",
        )
        with self.login_user_context(admin_user):
            response = self.client.get(
                URL_CMS_TRANSLATION_DELETE % page.pk, {"language": "de"}
            )
            ...
            self.assertRedirects(response, URL_CMS_PAGE)
            response = self.client.get(
                URL_CMS_TRANSLATION_DELETE % page.pk, {"language": "es-mx"}
            )
            self.assertEqual(response.status_code, 200)
            response = self.client.post(
                URL_CMS_TRANSLATION_DELETE % page.pk, {"language": "es-mx"}
    ...
    def test_change_dates(self):
        ...
        with self.settings(USE_TZ=False, TIME_ZONE="UTC"):
            ...
            with self.login_user_context(admin_user):
                ...
                self.assertNotEqual(
                    draft.publication_date.timetuple(), original_date.timetuple()
                )
                self.assertEqual(
                    draft.publication_date.timetuple(), new_date.timetuple()
                )
                self.assertEqual(
                    draft.publication_end_date.timetuple(), new_end_date.timetuple()
                )
                if original_end_date:
                    self.assertNotEqual(
                        draft.publication_end_date.timetuple(),
                        original_end_date.timetuple(),
                    )
        ...
        with self.settings(USE_TZ=True, TIME_ZONE="UTC"):
            ...
            new_end_date = timezone.localtime(timezone.now()) + datetime.timedelta(
                days=1
            )
            url = admin_reverse("cms_page_dates", args=(draft.pk,))
            with self.login_user_context(admin_user):
                ...
                self.assertEqual(
                    timezone.localtime(draft.publication_date).timetuple(),
                    new_date.timetuple(),
                )
                self.assertEqual(
                    timezone.localtime(draft.publication_end_date).timetuple(),
                    new_end_date.timetuple(),
                )
                if original_end_date:
                    self.assertNotEqual(
                        draft.publication_end_date.timetuple(),
    ...
    def test_change_template(self):
        ...
        with self.login_user_context(admin_user):
            response = self.client.post(
                self.get_admin_url(Page, "change_template", 1), {"template": template}
            )
    ...
    def test_unihandecode_doesnt_break_404_in_admin(self):
        ...
        if get_user_model().USERNAME_FIELD == "email":
            self.client.login(
                username="admin@django-cms.org", password="admin@django-cms.org"
            )
        else:
    ...
    def test_empty_placeholder_with_nested_plugins(self):
        ...
        page_en = create_page(
            "EmptyPlaceholderTestPage (EN)", "nav_playground.html", "en"
        )
...
class AdminTests(AdminTestsBase):
    ...
    def test_change_innavigation(self):
        ...
        with self.login_user_context(admin_user):
            request = self.get_request(post_data={"no": "data"})
            self.assertRaises(
                Http404, self.admin_class.change_innavigation, request, page.pk + 100
            )
        with self.login_user_context(permless):
            request = self.get_request(post_data={"no": "data"})
            response = self.admin_class.change_innavigation(request, page.pk)
    ...
    def test_publish_page_requires_perms(self):
        ...
        with self.login_user_context(permless):
            request = self.get_request()
            request.method = "POST"
            response = self.admin_class.publish_page(
                request, Page.objects.drafts().first().pk, "en"
            )
            self.assertEqual(response.status_code, 403)
    ...
    def test_preview_page(self):
        ...
        with self.login_user_context(permless):
            request = self.get_request()
            self.assertRaises(
                Http404, self.admin_class.preview_page, request, 404, "en"
            )
        page = self.get_page()
        page.publish("en")
        ...
        new_page = create_page(
            "testpage", "nav_playground.html", "fr", site=new_site, published=True
        )
        ...
        with self.login_user_context(permless):
            ...
            self.assertEqual(
                response["Location"],
                "%s?%s&language=en"
                % (base_url, get_cms_setting("CMS_TOOLBAR_URL__EDIT_ON")),
            )
            request = self.get_request()
            response = self.admin_class.preview_page(request, page.pk, "en")
            self.assertEqual(response.status_code, 302)
            ...
            self.assertEqual(
                response["Location"],
                "http://django-cms.org/fr/testpage/?%s&language=fr"
    ...
    def test_too_many_plugins_global_children(self):
        ...
        with self.settings(CMS_PERMISSION=False, CMS_PLACEHOLDER_CONF=conf):
            ...
            body = page.placeholders.get(slot="body")
            link = add_plugin(
                body, "LinkPlugin", "en", name="text", external_link="http://test.test/"
            )
            with self.login_user_context(admin_user):
                data = {
                    "plugin_type": "TextPlugin",
    ...
    def test_edit_title_dirty_bit(self):
        ...
        admin_url = reverse(
            "admin:cms_page_edit_title_fields", args=(draft_page.pk, language)
        )
...
class PluginPermissionTests(AdminTestsBase):
    ...
    def _get_change_page_request(self, user, page):
        return type(
            "Request",
            (object,),
            {"user": user, "path": base.URL_CMS_PAGE_CHANGE % page.pk},
        )
    ...
    def test_plugin_edit_wrong_url(self):
        ...
        url = "%s/edit-plugin/%s/" % (
            admin_reverse("cms_page_edit_plugin", args=[plugin.id]),
            plugin.id,
        )
...
class AdminFormsTests(AdminTestsBase):
    ...
    def test_missmatching_site_parent_dotsite(self):
        ...
        parent_page = api.create_page(
            "test", get_cms_setting("TEMPLATES")[0][0], "fr", site=new_site
        )
        new_page_data = {
            "title": "Title",
            "slug": "slug",
    ...
    def test_form_errors(self):
        ...
        page1 = api.create_page(
            "test", get_cms_setting("TEMPLATES")[0][0], "fr", site=site0
        )
        ...
        with self.login_user_context(superuser):
            ...
            response = self.client.post(self.get_admin_url(Page, "add"), new_page_data)
            expected_error = (
                '<ul class="errorlist"><li>Slug must not be empty.</li></ul>'
            )
            self.assertEqual(response.status_code, 200)
            self.assertContains(response, expected_error, html=True)
    ...
    def test_advanced_settings_endpoint(self):
        ...
        with self.login_user_context(admin_user):
            ...
            redirect_path = (
                admin_reverse("cms_page_change", args=(page.pk,)) + "?language=de"
            )
        ...
        with self.login_user_context(admin_user):
            ...
            self.assertRedirects(response, redirect_path)
            self.assertEqual(
                Page.objects.get(pk=page.pk).template, "nav_playground.html"
            )
    ...
    def test_create_page_type(self):
        page = create_page(
            "Test", "static.html", "en", published=True, reverse_id="home"
        )
        ...
        with self.login_user_context(superuser):
            ...
            new_page_id = (
                Page.objects.filter(is_page_type=True).only("pk").latest("id").pk
            )
    ...
    def test_smart_link_published_pages(self):
        ...
        page_url = (
            URL_CMS_PAGE_PUBLISHED  # Not sure how to achieve this with reverse...
        )
        ...
        with self.login_user_context(staff_guy):
            ...
            self.assertEqual(
                200,
                self.client.get(
                    page_url, HTTP_X_REQUESTED_WITH="XMLHttpRequest"
                ).status_code,
            )
        ...
        with self.settings(CMS_PERMISSION=True):
            ...
            with self.login_user_context(admin_user):
                ...
                get_user_model().objects.create_user(
                    username=USER_NAME,
                    email=USER_NAME + "@django-cms.org",
                    password=USER_NAME,
                )
                ...
                self.assertTrue(
                    page_size_grown, "Page size has not grown after user creation"
                )
