...
from cms.models import Page, CMSPlugin, ACCESS_DESCENDANTS
...
class PermissionModeratorTestCase(SettingsOverrideTestCase):
    ...
    def setUp(self):
        ...
        # create super user
        self.user_super = User(username="super", is_staff=True,
        ...
        # create master user
        self.user_master = User(username="master", is_active=True)
        self.user_master.set_password("master")
        self.user_master.save()
        self.user_master = create_page_user(self.user_master,
                                            self.user_master,
                                            grant_all=True)
        ...
        self.slave_page = create_page("slave", "nav_playground.html", "en",
                                      parent=self.master_page,
                                      created_by=self.user_super)
        self.user_slave = User(username="slave", is_active=True)
        self.user_slave.set_password("slave")
        self.user_slave.save()
        self.user_slave = create_page_user(self.user_slave, self.user_slave,
                                           can_add_page=True,
        ...
        # create page_b
        page_b = create_page("pageB", "nav_playground.html", "en",
                             created_by=self.user_super)

        # Normal user
        self.user_normal = User(username="normal", is_active=True)
        self.user_normal.set_password("normal")
        self.user_normal.save()
        self.user_normal = create_page_user(self.user_normal, self.user_normal)
        ...
        assign_user_to_page(page_a, self.user_master,
                            can_add=True, can_change=True,
                            can_delete=True, can_publish=True,
                            can_move_page=True, can_moderate=True)

        # publish after creating all drafts
        publish_page(self.home_page, self.user_master)
        publish_page(self.master_page, self.user_master)
        publish_page(page_b, self.user_normal)
    ...
    def test_05_slave_can_add_page_under_slave_home(self):
        ...
        page = create_page("page", "nav_playground.html", "en",
                           parent=slave_page, created_by=self.user_slave)
    ...
    def test_07_super_can_add_plugin(self):
        slave_page = Page.objects.get(pk=self.pages['slave'])
        self._add_plugin(self.user_super, page=slave_page)

    def test_08_master_can_add_plugin(self):
        slave_page = Page.objects.get(pk=self.pages['slave'])
        self._add_plugin(self.user_master, page=slave_page)

    def test_09_slave_can_add_plugin(self):
        slave_page = Page.objects.get(pk=self.pages['slave'])
        self._add_plugin(self.user_slave, page=slave_page)
    ...
    def test_16_moderator_flags(self):
        ...
        # publish slave page
        slave_page = publish_page(slave_page, self.user_master)
    ...
    def test_patricks_move(self):
        ...
        pg = approve_page(pg, self.user_master)
        pe = approve_page(pe, self.user_master)
        ph = approve_page(ph, self.user_master)
        pf = approve_page(pf, self.user_master)
