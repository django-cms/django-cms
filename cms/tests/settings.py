...
DATABASES = app_manage.DatabaseConfig(
    env="DATABASE_URL", arg="--db-url", default="sqlite://localhost/local.sqlite"
).get_value(sys.argv, os.environ)
...
MEDIA_ROOT = app_manage.TempDir().get_value(sys.argv, os.environ)
STATIC_ROOT = app_manage.TempDir().get_value(sys.argv, os.environ)
CMS_MEDIA_ROOT = app_manage.TempDir().get_value(sys.argv, os.environ)
