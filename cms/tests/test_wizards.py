...
class TestWizardBase(WizardTestMixin, TransactionCMSTestCase):
    ...
    def test_endpoint_auth_required(self):
        ...
        staff_active = self._create_user(
            "staff-active", is_staff=True, is_superuser=False, is_active=True
        )
...
class TestPageWizard(WizardTestMixin, CMSTestCase):
    ...
    def test_wizard_create_atomic(self):
        ...
        self.assertTrue(form.is_valid())
        self.assertFalse(
            Page.objects.filter(template=TEMPLATE_INHERITANCE_MAGIC).exists()
        )
    ...
    def test_create_page_with_existing_slug(self):
        ...
        create_page(
            "page", "nav_playground.html", language="en", published=True, slug="page"
        )
