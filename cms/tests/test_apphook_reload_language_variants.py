...
class ApphookReloadLanguageVariantTests(CMSTestCase):
    ...
    def test_new_language_variant_triggers_url_reload(self):
        ...
        with apphooks(SampleApp):
            ...
            self.assertNotEqual(
                initial_revision, 
                new_revision,
                "URL revision should change when creating a language variant for an apphook page"
            )

    def test_new_language_variant_without_apphook_no_reload(self):
        """
        Test that creating a new language variant for a page without an apphook
        does NOT trigger URL reloading.
        """
        ...
        self.assertEqual(
            initial_revision, 
            new_revision,
            "URL revision should NOT change when creating a language variant for a non-apphook page"
        )

    @patch('cms.signals.pagecontent.set_restart_trigger')
    def test_signal_handler_called_for_apphook_page(self, mock_set_restart_trigger):
        """
        Test that the signal handler is called when creating a language variant
        for a page with an apphook.
        """
        with apphooks(SampleApp):
    ...
    @patch('cms.signals.pagecontent.set_restart_trigger')
    def test_signal_handler_not_called_for_non_apphook_page(self, mock_set_restart_trigger):
        """
        Test that the signal handler is NOT called when creating a language variant
        for a page without an apphook.
        """
    ...
    @patch('cms.signals.pagecontent.set_restart_trigger')
    def test_signal_handler_not_called_for_existing_content_update(self, mock_set_restart_trigger):
        """
        Test that the signal handler is NOT called when updating existing PageContent
        (only when creating new ones).
        """
        with apphooks(SampleApp):
            # Create a page with an apphook
            page = create_page(
